  ...
  <script>
    var i = 0;
  	function addListItem(flag) {
  		if (flag === 1) {
  			$("#listView1").append('<li class="ui-state-default">List Item - ' + (++i) + '</li>').listview("refresh"); 
  		} else {
  			var container = $("#container1"), UL = container.find("#listView2");
  			if (UL.length) {
  				UL.append('<li class="ui-state-default">List Item - ' + (++i) + '</li>').listview("refresh");
  			} else {
  				container.append('<ul data-role="listview" id="listView2"><li class="ui-state-default">List Item - ' + (++i) + '</li></ul>').trigger("create");
  			} 
  		}
  	} 
  </script>
